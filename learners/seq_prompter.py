import torch
from utils import *
from impromptu.utils.dVAEomptu.utils.dVAE import dVAE
from SLATE import SLATE_encoder
from impromptu.utils.transformer import PositionalEncoding, TransformerDecoder, ContextEncoder
import argparse

class SeqPrompter(nn.Module):

	def __init__(self, args):
		'''
		Relational Slot Attention Network
		'''
		super().__init__()
	...
	# def get_context(self, slots):

	# 	'''
	# 	Gets relational representations from graph network
	# 	'''		
	# 	context = self.graph_network(slots) #B, num_slots, d_model
	# 	return context
	...
	def forward(self, support, query, tau, hard=False):
		...
		slots, attns, recon, _, emb_input, z_transformer_target = self.slate_encoder(all_inp, tau, hard)
		...
		#generate prompt
		prompt = torch.cat((encoded_context, encoded_inp), 1)
		#generate output
...
def main():
	...
	model = PromptNetwork(args)
	images = torch.randn(10,4,2,3,16,16)
	recon, ce, attns = model(images[:,:3],images[:,3], 1.0)
