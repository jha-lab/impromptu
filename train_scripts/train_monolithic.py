...
log_dir = os.path.join(args.log_path, args.dataset, filename[:-3] +'_monolithic_'+str(args.seed))
...
optimizer = Adam(model.parameters(), lr=args.lr_main)
...
for epoch in range(start_epoch, args.epochs):
    ...
    for batch, images in enumerate(train_loader):
        ...
        optimizer.param_groups[0]['lr'] = lr_decay_factor*lr_warmup_factor*args.lr_main

        images = images.cuda()
        B, _, _, C, H, W = images.shape
        images = augment_analogy(images)
        recon, mse = model(images[:,:-1],images[:,-1])
        optimizer.zero_grad()
        loss = mse 
        loss.backward()
        optimizer.step()
    ...
    with torch.no_grad():
        ...
        vis_recon = visualize_generation(images, recon)
        grid = vutils.make_grid(vis_recon, nrow=5, pad_value=0.2)#A:B::C:D gt(D)
    ...
    with torch.no_grad():
        ...
        if val_loss < best_val_loss:
            ...
            if epoch >= 20:
                out = model.generate(images[:,:-1],images[:,-1,0])
                vis_recon = visualize_generation(images, out)
                grid = vutils.make_grid(vis_recon, nrow=5, pad_value=0.2)#A:B::C:D gt(D)
